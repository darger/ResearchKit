/*
* This header is generated by classdump-dyld 0.7
* on Monday, March 9, 2015 at 8:56:15 PM Greenwich Mean Time
* Operating System: Version 8.1 (Build 12B411)
* Image Source: /private/var/mobile/Containers/Bundle/Application/59532D3A-E75D-48D6-95BE-6EFEA922624C/Parkinson.app/Frameworks/ResearchKit.framework/ResearchKit
* classdump-dyld is licensed under GPLv3, Copyright © 2013 by Elias Limneos.
*/

#import <ResearchKit/ORKStepViewController.h>

@class _ORKInstructionStepView;

@interface ORKInstructionStepViewController : ORKStepViewController {

	_ORKInstructionStepView* _stepView;

}

@property (nonatomic,retain) _ORKInstructionStepView * stepView;              //@synthesize stepView=_stepView - In the implementation block
-(void)_stepDidChange;
-(void)setContinueButtonItem:(id)arg1 ;
-(void)setLearnMoreButtonItem:(id)arg1 ;
-(void)setStepView:(_ORKInstructionStepView *)arg1 ;
-(void)useAppropriateButtonTitleAsLastBeginningInstructionStep;
-(id)instructionStep;
-(_ORKInstructionStepView *)stepView;
-(void)encodeRestorableStateWithCoder:(id)arg1 ;
-(void)decodeRestorableStateWithCoder:(id)arg1 ;
-(void)viewWillAppear:(BOOL)arg1 ;
-(void)viewDidLoad;
@end

