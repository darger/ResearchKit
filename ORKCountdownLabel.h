/*
* This header is generated by classdump-dyld 0.7
* on Monday, March 9, 2015 at 8:56:14 PM Greenwich Mean Time
* Operating System: Version 8.1 (Build 12B411)
* Image Source: /private/var/mobile/Containers/Bundle/Application/59532D3A-E75D-48D6-95BE-6EFEA922624C/Parkinson.app/Frameworks/ResearchKit.framework/ResearchKit
* classdump-dyld is licensed under GPLv3, Copyright © 2013 by Elias Limneos.
*/

#import <ResearchKit/ResearchKit-Structs.h>
#import <ResearchKit/ORKLabel.h>

@class NSString;

@interface ORKCountdownLabel : ORKLabel {

	NSString* _mmString;
	NSString* _ssString;

}

@property (nonatomic,retain) NSString * mmString;              //@synthesize mmString=_mmString - In the implementation block
@property (nonatomic,retain) NSString * ssString;              //@synthesize ssString=_ssString - In the implementation block
-(void)_setCountDownValue:(long long)arg1 ;
-(void)_renderText;
-(NSString *)mmString;
-(void)setMmString:(NSString *)arg1 ;
-(NSString *)ssString;
-(void)setSsString:(NSString *)arg1 ;
-(void)setFont:(id)arg1 ;
-(CGSize)intrinsicContentSize;
-(void)_updateAppearance;
@end

