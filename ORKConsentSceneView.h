/*
* This header is generated by classdump-dyld 0.7
* on Monday, March 9, 2015 at 8:56:14 PM Greenwich Mean Time
* Operating System: Version 8.1 (Build 12B411)
* Image Source: /private/var/mobile/Containers/Bundle/Application/59532D3A-E75D-48D6-95BE-6EFEA922624C/Parkinson.app/Frameworks/ResearchKit.framework/ResearchKit
* classdump-dyld is licensed under GPLv3, Copyright © 2013 by Elias Limneos.
*/

#import <ResearchKit/ORKVerticalContainerView.h>

@class ORKConsentSection;

@interface ORKConsentSceneView : ORKVerticalContainerView {

	ORKConsentSection* _consentSection;

}

@property (nonatomic,retain) ORKConsentSection * consentSection;              //@synthesize consentSection=_consentSection - In the implementation block
-(void)setConsentSection:(ORKConsentSection *)arg1 ;
-(ORKConsentSection *)consentSection;
@end

